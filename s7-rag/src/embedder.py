...
def truncate_to_512(text: str, max_tokens: int = 500) -> str:
    encoded = tokenizer(
        text, max_length=max_tokens, truncation=True, add_special_tokens=False
    )
...
def embed(html_doc_obj):
    ...
    store = PGVectorStore.create_sync(
        engine=engine, table_name=TABLE_NAME, embedding_service=custom_embeddings
    )
