import sys
import traceback

sys.path.append("./s13-torch-translation-tutorial/")


# WORKAROUND for SpaCy language model download. To use SpaCy models,
# the model must be downloaded and installed before the training script.
# TO make it dynamic, we call the spacy.cli.download() function to download the model
# to a specific location, and then import it from that location.
def ensure_lang_model(model_name: str):
    from importlib import import_module

    import spacy
    from spacy.cli import download

    OLD_MODEL_SHORTCUTS = (
        spacy.errors.OLD_MODEL_SHORTCUTS
        if hasattr(spacy.errors, "OLD_MODEL_SHORTCUTS")
        else {}
    )

    model_name = OLD_MODEL_SHORTCUTS.get(model_name, model_name)
    download(model_name, False, False, None, "-t", "/shared/language_models/")
    sys.path.append("/shared/language_models/")
    model_module = import_module(model_name)
    model_module.load()


from main import main


def train(
    project,
    run,
    training_data,
    validation_data,
    src_lang="de",
    tgt_lang="en",
    epochs=30,
    lr=1e-4,
    batch_size=128,
    backend="cpu",
    attn_heads=8,
    enc_layers=5,
    dec_layers=5,
    embed_size=512,
    dim_feedforward=512,
    dropout=0.1,
    model_name="translator-model",
):
    print("Running training script...")
    opts = type("Namespace", (), {})()
    opts.src = src_lang
    opts.tgt = tgt_lang
    opts.epochs = epochs
    opts.lr = lr
    opts.batch = batch_size
    opts.backend = backend
    opts.attn_heads = attn_heads
    opts.enc_layers = enc_layers
    opts.dec_layers = dec_layers
    opts.embed_size = embed_size
    opts.dim_feedforward = dim_feedforward
    opts.dropout = dropout

    model_dir = "./data/output/"
    # fixed logging dir
    opts.logging_dir = model_dir

    try:
        ensure_lang_model(src_lang)
        ensure_lang_model(tgt_lang)
    except Exception as e:
        print(f"Error downloading language models: {e}")
        print(traceback.format_exc())
        raise

    training_data.download("./data/input/training.tar.gz", overwrite=True)
    validation_data.download("./data/input/validation.tar.gz", overwrite=True)
    opts.train_file = "./data/input/training.tar.gz"
    opts.valid_file = "./data/input/validation.tar.gz"

    opts.dry_run = False

    opts.run = run

    try:
        metrics = main(opts)
        print(f"Model metrics:{metrics}")
    except Exception as e:
        print(f"Error running training script: {e}")
        print(traceback.format_exc())
        raise

    parameters = {
        "src": src_lang,
        "tgt": tgt_lang,
        "epochs": epochs,
        "lr": lr,
        "batch": batch_size,
        "backend": backend,
        "attn_heads": attn_heads,
        "enc_layers": enc_layers,
        "dec_layers": dec_layers,
        "embed_size": embed_size,
        "dim_feedforward": dim_feedforward,
        "dropout": dropout,
    }

    # log model
    model_artifact = project.log_model(
        name=model_name,
        framework="pytorch",
        source=model_dir + "best.pt",
        parameters=parameters,
    )
    model_artifact.log_metrics(metrics)
